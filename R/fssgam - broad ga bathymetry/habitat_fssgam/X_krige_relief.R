...
datn           <- nrow(habisp)
preddf         <- traind[, colnames(traind) %in% c("layer_depth", "layer_tpi",
                                                   "layer_roughness", 
                                                   "layer_slope",
                                                   "layer_detrended")]
...
relief_stack   <- inla.stack(data = list(y = traind$mean.relief),
                             ...
                             effects = list(c(sp = list(1:mesha$n)),
                                            list(depth = preddf$layer_depth,
                                                 rough = preddf$layer_roughness,
                                                 tpi   = preddf$layer_tpi,
                                                 slope = preddf$layer_slope,
                                                 dtren = preddf$layer_detrended)),
                             remove.unused = TRUE)

modform        <- y ~ 1 + depth + dtren + tpi + f(sp, model = spde)
